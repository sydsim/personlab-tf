...
def get_keypoints(hm, so_x, so_y):
    ...
    for i in range(config.NUM_KP):
        ...
        h_mf = np.logical_and(maximum_filter(h_gf, size=5) == h_gf, h_gf > 2.)
        for y, x in zip(*np.nonzero(h_mf)):
            kp_candidates.append((h_gf[y, x], y, x, i))
        ...
        while (len(q) > 0):
            ...
            for e_i, nodes in enumerate(config.EDGES):
                ...
                for p_j, dat in enumerate(kp_list):
                    _, py, px, pk = dat
                    if pk != e2 or include_in[p_j] is not None:
                        continue
